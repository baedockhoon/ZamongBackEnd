...
public class ArtistGropDAO {
	...
	public int insert(ArtistDTO dto) {
		int affected = 0;
		String sql = "INSERT INTO GP_ATGROUP VALUES(GP_SEQUENCE.NEXTVAL, SYSDATE, ?,?,?)";
		try {
	...
	public int groupInsert() {
		int affected = 0;
		String sql = "INSERT INTO GPM_GROUPMEMBER VALUES(GPM_GROPMEMBER_SEQ.NEXTVAL, SYSDATE, (SELECT MAX(AT_NO) FROM AT_ARTIST), (SELECT MAX(GP_NO) FROM GP_ATGROUP))";
		try {
			psmt = conn.prepareStatement(sql);
